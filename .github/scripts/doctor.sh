...
# WHAT IT WILL NOT DO
#
#   * Block. It is not on the commit hook and it never fails `make first-session`. A
#     missing variable is a fact about a machine, not about a commit.
...
#     0  everything declared is present, or there was nothing declared
#     1  something declared is missing or mismatched
#     2  could not run
#
# STEAL: A dependency-free environment parity check: declared variables against the
# environment, declared runtime pins against the interpreters actually on PATH, declared
# write targets against the filesystem. Reads names, never values. Depends on bash and
# coreutils only.
...
if [ ${#declared_vars[@]} -gt 0 ]; then
  ...
  if [ ${#missing[@]} -eq 0 ]; then
    note "  PASS  all ${#declared_vars[@]} declared variables are set"
  else
    fail "  FAIL  ${#missing[@]} declared variable(s) are not set in this environment:"
  ...
  if [ -f .env ]; then
    ...
    while IFS= read -r line; do
      ...
      found=0
      for declared in "${declared_vars[@]}"; do
...
if [ "$problems" -gt 0 ]; then
  ...
  echo "This reports and blocks nothing: it is not on the commit hook, and it does not"
  echo "stop 'make first-session'. In CI it turns the run red so somebody sees it."
  exit 1
fi

echo "Everything this project declares is present and matches."
echo "That is a statement about declared requirements only — a variable nobody wrote down"
echo "is not checked here, because nothing knows it exists."
exit 0
